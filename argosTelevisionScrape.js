...
function getTelevisionOffers(){
  ...
  var tvOffers = Array.prototype.map.call(tvOfferSections, function(tv){
    ...
    if(($(tv).find('dt.title').find('a').text())!=null){
      ...
      screenSize = properties.match(/\d{2}( In|In)/);
      if (screenSize!=null){
        ...
        if (properties.match(/LED/i)){
          tvType = "LED";
        }
        if (properties.match(/Plasma/i)){
          tvType = "Plasma";
        }
        if (properties.match(/LCD/i)){
          tvType = "LCD";
        }
...
casper.start("http://www.argos.co.uk/static/Browse/c_1/1%7Ccategory_root%7CTechnology%7C33006169/c_2/2%7C33006169%7CTelevisions+and+accessories%7C33008651/c_3/3%7Ccat_33008651%7CTelevisions%7C33017148/fs/2/p/1/pp/150/s/Promotions.htm", function() {
});
