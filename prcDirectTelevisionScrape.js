...
function getTelevisionOffers(){
  var tvOfferSections = $('div.product-listing-item');
  ...
  var tvOffers = Array.prototype.map.call(tvOfferSections, function(tv){
      ...
      if(($(tv).find('div.product-listing-text').find('p.small_text').text())!=null){
        description = $(tv).find('div.product-listing-text').find('p.small_text').text();
      ...
      if(($(tv).find('img').attr('src'))!=null){
        ...
        imageUrl = "http://www.prcdirect.co.uk" + imageUrl;
      }
      if(($(tv).find('p.listing_tsp').text())!=null){
      	originalPrice = $(tv).find('p.listing_tsp').text();
...
//  //   skimlinksUrl = 'http://go.redirectingat.com/?id=54354X1347041&site=www.specialoffershopper.co.uk&xs=1&isjs=1&url='+ encodedProductUrl + '&xguid=d1cbe13e65417e162e392fc0edcccb5f&xcreo=0&sref=http%3A%2F%2Fwww.specialoffershopper.co.uk&xtz=-660';
    skimlinksUrl = 'http://www.awin1.com/cread.php?awinmid=3631&awinaffid=193639&clickref=&p='+encodedProductUrl;
          ...
          for (var i=0; i < currysTelevisions.length; i++){
           // utils.dump(currysTelevisions[i]);
            if (currysTelevisions[i].type != undefined){
             ...
             fs.write('./prc direct-television.txt', televisionString + "\n", 'a');
             //utils.dump(currysTelevisions);
            }
         }
...
casper.start("http://www.prcdirect.co.uk/ajax_results.php?category_string=TV/televisions/page1/price/0_8949/TV/televisions&setting=items_per_page&value=ALL", function() {
