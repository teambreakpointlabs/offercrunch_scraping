...
function getTabletOffers(){
  var tabletOfferSections = $('.product.productCompare');
  ...
  var tabletOffers = Array.prototype.map.call(tabletOfferSections, function(tablet){
      ...
      var encodedProductUrl = '';
    	 ...
    	 if(($(tablet).find('.productTitle').find('a').attr('href'))!=null){
    	 	 productUrl = $(tablet).find('.productTitle').find('a').attr('href');
       }
       if(($(tablet).find('img.image.respImg').attr('src'))!=null){
         imageUrl = $(tablet).find('img.image.respImg').attr('src');
       }
      if(($(tablet).find('header.productTitle').find('a').text())!=null){
    		properties = $(tablet).find('header.productTitle').find('a').text();
      }
 ...
 if(oldPrice!=null){
  ...
  // tabletOffer.details = details;
   ...
   tabletOffer.description = properties;
    ...
    utils.dump(currysTablets);
          for (var i=0; i < currysTablets.length; i++){
            ...
            if (currysTablets[i].type != undefined){
             ...
             fs.write('./currys-camera.txt', tabletString + "\n", 'a');
            // utils.dump(currysTablets[i]);
           }
...
casper.start("http://www.currys.co.uk/gbuk/cameras/digital-cameras/compact-digital-cameras/344_3199_30301_xx_xx/xx-criteria.html", function() {
