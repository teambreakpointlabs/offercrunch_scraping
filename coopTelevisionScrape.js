...
function getTelevisionOffers(){
  ...
  var tvOffers = Array.prototype.map.call(tvOfferSections, function(tv){
    ...
    skimlinksUrl = 'http://www.awin1.com/cread.php?awinmid=1915&awinaffid=193639&clickref=&p='+encodedProductUrl;
    ...
    if (argosTelevisions!=null){
      for (var i=0; i < argosTelevisions.length; i++){
          // if (argosTelevisions[i].type != undefined){
            ...
            fs.write('./coop electrical-television.txt', televisionString + "\n", 'a');
...
casper.start("http://www.coopelectricalshop.co.uk/TV+DVD+and+Bluray/Televisions/LED+TV!APO!s/fd/c/c/c?pgsz=96", function() {
});
