...
function getTelevisionOffers(){
  var tvOfferSections = $('ul.product-list>li');
  ...
  var tvOffers = Array.prototype.map.call(tvOfferSections, function(tv){
    ...
    if(($(tv).find('.product-list-cnt-mid').find('h2').find('a').attr('href'))!=null){
      productUrl = $(tv).find('.product-list-cnt-mid').find('h2').find('a').attr('href');
    }
...
casper.start("http://www.hughesdirect.co.uk/vision/televisions/all/productlist", function() {
});
