...
function getTelevisionOffers(){
  ...
  var tvOffers = Array.prototype.map.call(tvOfferSections, function(tv){
    ...
    var brands = ['Blaupunkt','Cello','Digihome','Foehn & Hirsch','Furrion','Goodmans','Hannspree','LG','Panasonic','Philips','Samsung','Sharp','Sony','Toshiba'];
     ...
     if(($(tv).find('.title-holder').find('h3').text()!=null)){
       ...
       if (screenSize != null){
        ...
        if (screenSize!=null){
          screenSize = parseInt(screenSize[0]);
        }
       }
       if (screenSize==null){
        screenSize = description.match(/\d{2}(in|-inch)/);
        if (screenSize!=null){
          screenSize = parseInt(screenSize[0]);
        }
       }
    ...
    if (currysTelevisions!=null){
          ...
          // utils.dump(currysTelevisions);
           ...
           // utils.dump(currysTelevisions[i]);
            ...
            fs.write('./ebuyer-television.txt', televisionString + "\n", 'a');
             utils.dump(currysTelevisions[i]);
            }
          }
      }
...
casper.start("http://www.ebuyer.com/store/Visual/cat/Televisions?limit=30&page=1", function() {
