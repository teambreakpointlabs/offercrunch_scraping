...
function getShoeOffers(){
  var shoeOfferSections = $('.product-list-item');
  ...
  var shoeOffers = Array.prototype.map.call(shoeOfferSections, function(shoe){
    ...
    if(($(shoe).find('.product').find('a').attr('href'))!=null){
      productUrl = $(shoe).find('.product').find('a').attr('href');
      productUrl = "http://www.gant.co.uk"+productUrl;
    }
    if(($(shoe).find('span.name').text())!=null){
      description = $(shoe).find('span.name').text();
    ...
    if(($(shoe).find('del.was').text())!=null){
        originalPrice = $(shoe).find('del.was').text();
    ...
    if(($(shoe).find('ins.now').text())!=null){
      
     offerPrice = $(shoe).find('ins.now').text();
    ...
    skimlinksUrl = 'http://www.awin1.com/cread.php?awinmid=5643&awinaffid=193639&clickref=&p='+encodedProductUrl;
    ...
    if (asosMensShoes!=null){
      for (var i=0; i < asosMensShoes.length; i++){
           if (asosMensShoes[i].type != undefined){
            ...
            fs.write('./gant-shirt.txt', shoeString + "\n", 'a');
...
casper.start("http://www.gant.co.uk/mens-shirts-sale/", function() {
